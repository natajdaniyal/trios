...
def test_parameter_rule_stores_definition():
    rule = ParameterRule("mass", True, 1.0, minimum=1.0, maximum=5.0, step=0.5)
...
def test_fixed_parameter_only_accepts_default():
    rule = ParameterRule("mass", False, 2.0)
...
def test_editable_parameter_validates_range_and_step():
    rule = ParameterRule("mass", True, 1.0, minimum=1.0, maximum=3.0, step=0.5)
