...
def test_run_experiment_returns_experiment_result():
    ...
    experiment.add_stage(
        ExperimentStage(
            "stage-1",
            make_configuration(make_body("A")),
        )
    )
...
def test_run_experiment_preserves_stage_order():
    experiment = Experiment("test-experiment")
    experiment.add_stage(
        ExperimentStage("first", make_configuration(make_body("A")))
    )
    experiment.add_stage(
        ExperimentStage("second", make_configuration(make_body("B")))
    )

    result = run_experiment(experiment, steps_per_stage=0)

    assert [stage["stage_name"] for stage in result.data["stages"]] == [
        "first",
        "second",
    ]
